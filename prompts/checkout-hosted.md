Add a hosted Mayar checkout to this project.

Goal:
- When the buyer clicks buy, send them to the Mayar checkout page at the
  documented `link`. Mayar owns the form, the payment methods, and the
  success page.
- Checkout type is `hosted`. Do not ask me to pick hosted, embedded, or
  native.
...
- Checkout types, raw:
  https://raw.githubusercontent.com/mayarid/skills/main/skills/mayar-v2/references/checkout-types.md
...
5. Record checkout type as `hosted`. Ask me these before you write any plan,
...
7. Verify in sandbox and report. Prove that the client assigns the documented
   `link`. Tell me exactly which results you proved and which you could not,
   list the environment variables I must set, and give me the steps to switch
   to a production key at https://web.mayar.id/api-keys.
