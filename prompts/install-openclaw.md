# Install the Mayar Skill in OpenClaw

Install the official `mayar-v2` Agent Skill for OpenClaw, prove that this
session can see it, then stop. Do not start an integration. Do not require me to
edit this prompt.

## 1. Confirm the client

This prompt targets **OpenClaw** only. Confirm that you run in OpenClaw.
...
Do not use `openclaw skills install` for this repository. That command expects
`SKILL.md` at the root of the source. In `mayarid/skills` the file is at
`skills/mayar-v2/SKILL.md`, so the install fails. Use the `skills` CLI:

```bash
npx skills add mayarid/skills --skill mayar-v2 --agent openclaw --global --yes
```

The command installs `mayar-v2` to `~/.openclaw/skills/mayar-v2`. It reads the
...
Read the frontmatter of `~/.openclaw/skills/mayar-v2/SKILL.md`. Confirm that
`name` is `mayar-v2`.
...
## 4. Prove that OpenClaw sees the skill
...
Confirm that `mayar-v2` appears for `openclaw`. Do not list the directory, do
not run a second listing, and do not add your own checks.

`~/.openclaw/skills` is the managed skills directory. OpenClaw loads it for all
local agents that use that state. If the skill does not appear in the client,
tell me to restart OpenClaw.

This command proves that the files are in place. It does not prove that OpenClaw
...
> Check your skill list in OpenClaw for `mayar-v2`. Tell me if it is missing.

That line is required. It is the only proof that OpenClaw loaded the skill, and
